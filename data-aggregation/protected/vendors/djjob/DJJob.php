    ...
    public function start() {
        ...
        try {
            while ($this->count == 0 || $count < $this->count) {
                ...
                if (!$job) {
                    $this->log("[JOB] could not to get a job, queue::{$this->queue} may be empty", self::DEBUG);
                    sleep($this->sleep);
                    continue;
                }
...
class DJJob extends DJBase {
    ...
    public function acquireLock() {
        ...
        $query = " UPDATE "   . DJBase::_TABLE_NAME . 
                 ...
                 " AND (locked_at IS NULL OR locked_by = ?) " .
                 " AND failed_at IS NULL ";
        $lock = $this->runUpdate($query, $params);
        ...
        if (!$lock) {
          ...
          $this->log("[JOB] has been acquired lock for job::{$this->job_id}\n", self::INFO);
    ...
    public function getAttempts() {
        $rs = $this->runQuery(
            "SELECT attempts FROM "   . DJBase::_TABLE_NAME. "  WHERE id = ?",
            array($this->job_id)
        );
    ...
    public static function enqueue($handler, $queue = "default", $run_at = null) {
        $affected = self::runUpdate(
            "INSERT INTO "   . DJBase::_TABLE_NAME. "  (handler, queue, run_at, created_at) VALUES(?, ?, ?, NOW())",
    ...
    public static function bulkEnqueue($handlers, $queue = "default", $run_at = null) {
        $sql = "INSERT INTO "   . DJBase::_TABLE_NAME. "  (handler, queue, run_at, created_at) VALUES";
