...
class DJWorker extends DJBase {
    ...
    public function start() {
        ...
        $this->log("[JOB] worker shutting down after running {$job_count} {$this->tableJob}, over {$count} polling iterations", self::INFO);
    }
}
...
class DJJob extends DJBase {
    ...
    public function finish() {
        $this->runUpdate(
            "DELETE FROM {$this->tableJob} WHERE id = ?",
            array($this->job_id)
        );
    ...
    public function getHandler() {
        $rs = $this->runQuery(
            "SELECT handler FROM {$this->tableJob} WHERE id = ?",
            array($this->job_id)
        );
    ...
    public function getAttempts() {
        $rs = $this->runQuery(
            "SELECT attempts FROM {$this->tableJob} WHERE id = ?",
            array($this->job_id)
        );
    ...
    public static function enqueue($handler, $queue = "default", $run_at = null) {
        $affected = self::runUpdate(
            "INSERT INTO {$this->tableJob} (handler, queue, run_at, created_at) VALUES(?, ?, ?, NOW())",
    ...
    public static function bulkEnqueue($handlers, $queue = "default", $run_at = null) {
        $sql = "INSERT INTO {$this->tableJob} (handler, queue, run_at, created_at) VALUES";
        ...
        if ($affected < 1) {
            self::log("[JOB] failed to enqueue new {$this->tableJob}", self::ERROR);
            return false;
        }

        if ($affected != count($handlers))
            self::log("[JOB] failed to enqueue some new {$this->tableJob}", self::ERROR);
